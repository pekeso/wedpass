...
export async function DELETE(
  ...
  try {
    ...
    const data = await deleteMediaItem(weddingId, mediaId, userId)
    ...
    console.error("[DELETE /api/v1/weddings/:weddingId/media/:mediaId]", error)
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    const { userId } = requireAuth(request)
    const { weddingId, mediaId } = await params
    const data = await getMediaDownloadUrl(weddingId, mediaId, userId)
    return NextResponse.json({ success: true, data })
  } catch (error) {
    if (error instanceof UnauthorizedError) {
      return NextResponse.json(
    ...
    console.error("[GET /api/v1/weddings/:weddingId/media/:mediaId]", error)
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_SERVER_ERROR", message: "An error occurred" } },
      { status: 500 }
    )
